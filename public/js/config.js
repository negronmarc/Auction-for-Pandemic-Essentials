    ...
    return new Promise(function (resolve, reject) {
      Config.find({}, function (err, configs) {
        ...
        if (configs.length == 1) {
          ...
          if (options.max_watch_list) c.max_watch_list = options.max_watch_list;
          if (options.max_bid_count) c.max_bid_count = options.max_bid_count;
          if (options.max_offer_count)
            c.max_offer_count = options.max_offer_count;
          if (options.default_duration)
            c.default_duration = options.default_duration;
          if (options.image_size) c.image_size = options.image_size;
          if (options.max_image_count)
            c.max_image_count = options.max_image_count;
          if (options.image_type) c.image_type = options.image_type;
          if (options.step_price) c.step_price = options.step_price;
  ...
  loadConfig: function () {
    return new Promise(function (resolve, reject) {
      Config.find({}, function (err, configs) {
        ...
        if (configs.length > 1)
          return reject(
            "Too many config count(" +
              configs.length +
              "), need only one config doc"
          );
        if (configs.length == 0) {
          createConfig({}).then(
            function (c) {
